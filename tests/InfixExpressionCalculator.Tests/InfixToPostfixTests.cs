...
public class The_InfixToPostfix_Method
{
	...
	{
		Assert.Equal(postfix, InfixExpressionCalculator.InfixToPostfix(infix));
	}
	...
	{
		var exception = Assert.Throws<ArgumentException>(() => InfixExpressionCalculator.InfixToPostfix(infix));
	...
	{
		var exception = Assert.Throws<Exception>(() => InfixExpressionCalculator.InfixToPostfix(infix));
